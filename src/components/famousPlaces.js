...
export default function FamousPlaces({ famousPlacesData }) {
  const navigation = useNavigation();
  return (
    <View style={{ marginHorizontal: wp(4), marginBottom: hp(3) }}>
      <View>
        {famousPlacesData.length === 0 ? (
          <Loading size="large" style={{ marginTop: hp(5)  }} />
        ) : (
          <View>
            {famousPlacesData.map((item, index) => (
            //   <famousPlacesCard key={item.id} item={item} index={index} navigation={navigation} />
...
const famousPlacesCard = ({ item, index, navigation }) => {
    return (
